...
describe('process_jellyfin_command', () => {
  ...
  const commands = [
    {
      ...
      expected_url: get_jellyfin_url(`applemusic/now/${identifier}`, 'applemusic'),
    },
    {
    ...
    },
    {
      ...
      expected_url: get_jellyfin_url(`tunein/now/tunein:${identifier}`, 'tunein'),
    },
    {
      service: 'jellyfin Playlist',
      input: `playlist:${identifier}`,
      expected_url: get_jellyfin_url(`playlist/${identifier}`, 'jellyfin_playlist'),
    },
  ];
  ...
  test('Processes room change commands', async () => {
    ...
    expect(log).toHaveBeenCalledWith(`jellyfin room changed to ${jellyfin_room}`);
  });
